...
RSpec.describe Ukiryu::Executor do
  ...
  describe '.execute' do
    ...
    context 'with allow_failure option' do
      it 'returns Result with non-zero exit status instead of raising' do
        ...
        result = if Ukiryu::Platform.windows?
                   ...
                   executor.execute('sh', ['-c', 'exit 42'], allow_failure: true, shell: shell_symbol)
                 end
      ...
      it 'raises ExecutionError when command fails and allow_failure is false' do
        ...
        if Ukiryu::Platform.windows?
          expect do
            executor.execute('cmd', ['/c', 'exit 1'], shell: :cmd)
          end.to raise_error(Ukiryu::ExecutionError, /Command failed/)
        else
          expect do
            executor.execute('sh', ['-c', 'exit 1'], shell: shell_symbol)
          end.to raise_error(Ukiryu::ExecutionError, /Command failed/)
        end
      end
      ...
      it 'includes stderr in error message' do
        ...
        if Ukiryu::Platform.windows?
          ...
          expect do
            executor.execute('cmd', ['/c', 'echo error 1>&2 && exit 1'], shell: :cmd)
          end.to raise_error(Ukiryu::ExecutionError, /STDERR:\s*error/)
        else
          expect do
            executor.execute('sh', ['-c', 'echo error >&2; exit 1'], shell: shell_symbol)
          end.to raise_error(Ukiryu::ExecutionError, /STDERR:\s*error/)
        end
      end
    end
    ...
    context 'error formatting' do
      it 'includes executable name in error message' do
        expect do
          executor.execute('false', [], shell: shell_symbol)
      ...
      it 'includes exit status in error message' do
        ...
        if Ukiryu::Platform.windows?
          expect do
            executor.execute('powershell', ['-Command', 'exit 42'], shell: :powershell)
          end.to raise_error(Ukiryu::ExecutionError, /Exit status: 42/)
        else
          expect do
            executor.execute('sh', ['-c', 'exit 42'], shell: shell_symbol)
          end.to raise_error(Ukiryu::ExecutionError, /Exit status: 42/)
        end
      end
    end
    ...
    context 'command building' do
      it 'builds appropriate command for the shell' do
        ...
        result = if Ukiryu::Platform.windows?
                   ...
                   executor.execute('echo', ['test'], allow_failure: true, shell: shell_symbol)
                 end
        expect(result.exit_status).to eq(0)
      ...
      it 'includes executable path in CommandInfo' do
        result = executor.execute('echo', ['test'], allow_failure: true, shell: shell_symbol)
  ...
  describe 'integration tests' do
    it 'handles multiple rapid executions' do
      results = []
      5.times do
        results << executor.execute('echo', ['test'], allow_failure: true, shell: shell_symbol)
      end
