  ...
  describe '.execute with stdin' do
    ...
    context 'with string stdin data' do
      it 'passes stdin data to the command' do
        result = described_class.execute(executable, args, stdin: 'test data', shell: shell_symbol)
      ...
      it 'passes multi-line stdin data' do
        stdin_data = "line1\nline2\nline3"
        result = described_class.execute(executable, args, stdin: stdin_data, shell: shell_symbol)
    ...
    context 'with IO object stdin' do
      it 'reads from file IO object' do
        Tempfile.create('stdin_test') do |file|
          ...
          result = described_class.execute(executable, args, stdin: file, shell: shell_symbol)
      ...
      it 'reads from pipe IO object' do
        ...
        result = described_class.execute(executable, args, stdin: reader, shell: shell_symbol)
